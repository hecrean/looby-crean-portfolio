...
let oldDetailledScreenOrientation: detailledOrientation | undefined =
  detailledScreenOrientation;
...
function determineViewportSizeAndScreenOrientation(): void {
  ...
  if (Poller != null) {
    // we are still polling because of former event
    stopPolling();
    submitEvents();
    rememberSettings();
  }
  ...
  if (
    ...
    // viewport size changed in time => do not poll
    submitEvents();
    rememberSettings();
  }
}
...
export default {
  ...
  get detailledOrientation() {
    return detailledScreenOrientation;
  },
};
